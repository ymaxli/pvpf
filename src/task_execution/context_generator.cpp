    ...
    namespace task_execution {
        void figure_out_json_object(rapidjson::Document &conf, scheduler &sch) {
            for (Value::ConstValueIterator source = conf["source"].Begin();
                ...
                sch.json_object_map[(*source)["id"].GetString()] = const_cast<Value *>(source);
            }

            for (Value::ConstValueIterator node = conf["graph"].Begin();
                 node != conf["graph"].End(); node++) {
                sch.json_object_map[(*node)["id"].GetString()] = const_cast<Value *>(node);
            }

            for (Value::ConstValueIterator sink = conf["sink"].Begin(); sink != conf["sink"].End(); sink++) {
                sch.json_object_map[(*sink)["id"].GetString()] = const_cast<Value *>(sink);
            }
...
//            }
        ...
        shared_ptr<context>
        create_context(rapidjson::Value const &obj, scheduler &sch) {
...
//          add to pre{}
            ...
            if (obj.HasMember("input")) {
                ...
                for (Value::ConstValueIterator pre_it = pre_list.Begin(); pre_it != pre_list.End(); pre_it++) {
                    ...
                    pre.push_back(id);
                    if (sch.json_object_map.count(id) > 0) {
                        if ((*(sch.json_object_map[id]))["task"].HasMember("algorithm")) {
                            pre_is_cpu.push_back(is_cpu((*sch.json_object_map[id])["task"]["algorithm"].GetString()));
                        } else {
